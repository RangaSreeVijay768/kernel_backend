...
def generate_toc_pdf_final():
    try:
        input_excel_path = request.form.get('input_path')
        output_pdf_path = request.form.get('output_path')

        if not input_excel_path or not output_pdf_path:
            return jsonify({'error': 'Both input_excel_path and output_pdf_path are required'}), 400

        generate_clean_pdf(input_excel_path, output_pdf_path)
        return jsonify({'message': f'✅ PDF generated at {output_pdf_path}'}), 200
...
def enable_excel_editing(file_path: str):
    """
    Removes Windows Protected View flag if the file is blocked.
    This simulates "Enable Editing" by removing 'Zone.Identifier'.
    """
    ads_path = file_path + ":Zone.Identifier"
    
    # Method 1: Try deleting the alternate data stream (if exists)
    try:
        if os.path.exists(ads_path):
            os.remove(ads_path)
            print(f"✅ Removed Zone.Identifier from: {file_path}")
        else:
            # Also try deleting using Windows API in case it's hidden
            ctypes.windll.kernel32.DeleteFileW(f"{ads_path}")
            print(f"✅ Attempted Windows ADS deletion on: {file_path}")
    except Exception as e:
        print(f"⚠️ Error while unblocking file: {e}")
...
def generate_clean_pdf(input_excel_path: str, output_pdf_path: str):
    ...
    # === Save Final PDF ===
    final_pdf_path = os.path.join(output_pdf_path, "37111_MWH_TOC_Ver2_0_0.pdf")
